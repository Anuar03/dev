...
public class ReloadFromDb implements Reload {
    ...
    public ReloadFromDb(String dbeg, String dend) {
        this.documentIds = new ArrayList<>();
        this.rowsCount = new HashMap<>();
        this.reestrModels = new ArrayList<>();
        this.dbeg = dbeg;
    ...
    private void process() {
        try {
            ...
            if (!documentIds.isEmpty()) {
                long iter = getRange(documentIds.size());
...
//            con.close();
    ...
    private String sqlDynCount(int index) {
        ...
        int from = index * 1000;
        int to = from + 1000;
        System.out.println("Interval = " + from + "-" + to + " for period " + this.dbeg + " - " + this.dend);
        List<String> subDocumentIds;
        try {
            subDocumentIds = documentIds.subList(from, to);
        } catch (IndexOutOfBoundsException e) {
            subDocumentIds = documentIds.subList(from, documentIds.size());
        }
