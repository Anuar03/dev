    ...
    <div class="w-75 h-100 mx-auto pt-4">
        <div class="w-100 h-75 border-top border-secondary">
                ...
                <div id="dates-container">
                    ...
                    <div class="form-check-inline custom-radio">
                        <label class="form-check-label">
                            <input type="radio" class="form-check-input" @change="obChange" name="optradio">объектов проверок
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="row w-100 mb-2">
                <div class="col-3">
                    <div class="custom-control-inline custom-radio ml-4">
                        <input type="radio" class="custom-control-input" id="defaultUnchecked3" name="ff" @change="onNameChange">
                        <label class="custom-control-label" for="defaultUnchecked3">{{inpName}}</label>
                    </div>
                </div>
                <div class="col-9">
                    <div class="w-100" v-if="isName">
                        <div v-if="isIsp">
                            <label class="mr-sm-2" for="nameOblSelect">Область</label>
                            <select class="custom-select mb-2 mr-sm-2 mb-sm-0" id="nameOblSelect" v-model="state" @change="sortRegions">
                                <option selected>Выбор...</option>
                                <option v-for="(state, key) in states" v-bind:value="key">{{ key }}</option>
                            </select>
                            <label class="mr-sm-2" for="nameRegSelect">Район/Город</label>
                            <select class="custom-select mb-2 mr-sm-2 mb-sm-0" id="nameRegSelect" v-model="region">
                                <option selected>Выбор...</option>
                                <option v-for="(region, key) in sortedRegions" v-bind:value="key">{{region.value}}</option>
                            </select>
                            <label for="formGroupExampleInput">Фрагмент из наименования</label>
                            <v-select id="formGroupExampleInput1" v-model="test" :options="options">
                            </v-select>
<!--                            <input type="text" class="form-control" id="formGroupExampleInput" placeholder="Ввод..." v-model="fragText">-->
                        </div>
                        <div v-else>
                            <label class="mr-sm-2" for="objOblSelect">Раздел объекта</label>
                            <select class="custom-select mb-2 mr-sm-2 mb-sm-0" id="objOblSelect" v-model="objSection" @change="sortPodRazdel">
                                <option selected>Выбор...</option>
                                <option v-for="(val, key) in objRazdelList" v-bind:value="key">{{val}}</option>
                            </select>
                            <label class="mr-sm-2" for="objRegSelect">Подраздел объекта</label>
                            <select class="custom-select mb-2 mr-sm-2 mb-sm-0" id="objRegSelect" v-model="objSubsection" @change="sortObjName">
                                <option selected>Выбор...</option>
                                <option v-for="(val, key) in sortedObjPodRazdelList" v-bind:value="key">{{val.value}}</option>
                            </select>
                            <label class="mr-sm-2" for="objRegSelect1">Объект</label>
                            <select class="custom-select mb-2 mr-sm-2 mb-sm-0" id="objRegSelect1" v-model="objValue">
                                <option selected>Выбор...</option>
                                <option v-for="(val, key) in sortedObjList" v-bind:value="key">{{val.value}}</option>
                            </select>
                            <label for="formGroupExampleInput1">Фрагмент из наименования</label>
                            <input type="text" class="form-control" id="formGroupExampleInput1" placeholder="Ввод..." v-model="fragText">
            ...
            <div class="row w-100 mb-2">
                <div class="col-3">
                    <div class="custom-control custom-radio">
                        <input type="checkbox" class="custom-control-input" id="defaultChecked2" name="defaultExample2" v-model="isAllUsers">
                        <label class="custom-control-label" for="defaultChecked2">Все исполнители</label>
                    </div>
                </div>
                <div class="col-9">
                    <div class="w-100" v-if="!isAllUsers">
                        <label class="mr-sm-2" for="users">Выбор Исполнителя</label>
                        <select class="custom-select mb-2 mr-sm-2 mb-sm-0" id="users" v-model="lab">
                            <option selected>Выбор...</option>
                            <option v-for="(val, key) in labList" v-bind:value="key">{{val}}</option>
                        </select>
                    </div>
                </div>
            </div>
            <div class="row w-100 border-bottom border-secondary pb-2">
                <div class="w-100 text-center">
                    <button :disabled="isDisabled" type="button" class="btn btn-primary" @click="sendDataAndGetFile(true)">{{message}}</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            obChange() {
                this.inpOblName = 'По области объекта';
                this.inpRegName = 'По району объекта';
                this.inpName = 'По объекту';
                this.isIsp = false;
            },
            onOblChange() {
                this.isReg = false;
            ...
            sendDataAndGetFile(newVersion) {
                console.log(this.states);
                this.message = 'Подождите';
                this.isDisabled = true;
                axios({
                    url: 'generateReport/reestrFinishedWork',
                    method: 'POST',
                    responseType: 'blob',
                    data: {
